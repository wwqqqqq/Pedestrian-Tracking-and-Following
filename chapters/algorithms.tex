...
\subsubsection{相关滤波}
  ...
  KCF算法中，首先考虑使用线性拟合得到通用的预测公式，这里使用岭回归（Ridge Regression）以得到一个简单的闭式解。通过训练得到一个函数$f(\vec{z})=\vec{w}^T\vec{z}$，使得样本$\vec{x}_i$及其回归目标$y_i$之间的方差最小，即：
...
$$\vec{w}=(X^HX+\lambda I)^{-1}X^H\vec{y}$$
  ...
  为了说明，这里之一单通道一维信号为例，说明循环移位在此的用法。假设一个$n\times1$的向量$\vec{x}$可用于表示目标物所在的图像区域，$\vec{x}$即作为基础样本。为了训练一个分类器，我们使用基础样本作为正样本，并通过将$\vec{x}$进行移位，来获得一些虚拟的负样本。可以通过循环移位算子$P$来计算向量的一维平移，$P$是一个置换矩阵：
$$
P = 
\left[
\begin{matrix}
...
$$
  ...
  矩阵$P$与向量$\vec{x}$的乘积为$[x_n,x_1,x_2,\dots,x_{n-1}]^T$，将$\vec{x}$向右进行了一位的循环移位，$P^u\vec{x}$即可以计算出把$\vec{x}$移位$u$位的结果。所有通过这种方法移位的结果的集合为$\{P^u\vec{x}|u=0,\dots,n-1\}$。由于循环移位的特性，我们可以将该集合的前半部分视为正方向的移位，后半部分视为负方向的移位。
